...
def create_dual_pdf(
    ...
    if page_indices is not None and trans_count == len(page_indices):
        ...
        min_pages = min(orig_count, trans_count)
        for i in range(min_pages):
            doc_dual.insert_pdf(doc_orig, from_page=i, to_page=i)
            doc_dual.insert_pdf(doc_trans, from_page=i, to_page=i)

        if orig_count > min_pages:
            doc_dual.insert_pdf(doc_orig, from_page=min_pages, to_page=orig_count - 1)
        elif trans_count > min_pages:
            doc_dual.insert_pdf(doc_trans, from_page=min_pages, to_page=trans_count - 1)

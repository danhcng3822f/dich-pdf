...
class PDFPageInterpreterEx(PDFPageInterpreter):
    """Processor for the content of a PDF page with operator interception and patching."""

    def __init__(
        self,
        rsrcmgr: PDFResourceManager,
        device: PDFDevice,
        obj_patch: Optional[Dict[Any, str]] = None,
    ) -> None:
        super().__init__(rsrcmgr, device)
        self.rsrcmgr = rsrcmgr
        self.device = device
        self.obj_patch: Dict[Any, str] = obj_patch if obj_patch is not None else {}
    ...
    def do_S(self) -> Optional[str]:
        """Stroke path - filter non-formula lines."""
    ...
    def do_f(self) -> None:
        """Fill path using nonzero winding number rule (filtered)."""
        self.curpath = []
    ...
    def do_f_a(self) -> None:
        """Fill path using even-odd rule (filtered)."""
        self.curpath = []

    def do_B(self) -> None:
        """Fill and stroke path using nonzero winding number rule (filtered)."""
        self.curpath = []

    def do_B_a(self) -> None:
        """Fill and stroke path using even-odd rule (filtered)."""
        self.curpath = []
    ...
    def do_Do(self, xobjid_arg: PDFStackT) -> None:
        """Invoke named XObject and intercept stream patching."""
    ...
    def process_page(self, page: PDFPage) -> None:
        """Process page content streams and patch with new translated layout."""
        ...
        page_key = getattr(page, "page_xref", getattr(page, "pageno", 0))
        self.obj_patch[page_key] = (
    ...
    def render_contents(
        ...
        """Render content streams and return base operator string."""
        self.init_resources(resources)
        self.init_state(ctm)
        return self.execute(list_value(streams))

    def execute(self, streams: Sequence[object]) -> str:
        """Execute stream tokens, filtering out original text and recording non-text ops."""
        ops = ""
        try:
